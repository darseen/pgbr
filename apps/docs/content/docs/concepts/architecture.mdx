...
flowchart LR
    ...
    Dashboard -->|enqueue| Redis[(Redis<br/>BullMQ queues)]
    Redis -->|consume| Worker
    Dashboard -->|metadata| Postgres[(Postgres<br/>pgbr metadata)]
...
A Node process consuming three BullMQ queues. It spawns `pg_dump`, `pg_restore`,
and `psql` as child processes, streams the result to object storage, and records
the outcome in Postgres.
...
| Queued jobs, cron schedulers | **Redis** | Derived state — rebuildable. |
...
Redis being *derived* is deliberate. Schedules live in Postgres; the BullMQ job
schedulers in Redis are a projection of them. Every worker reconciles that
projection on boot, so a flushed Redis heals itself rather than silently dropping
your schedules.
...
- **A worker dies mid-job** — BullMQ's stalled-job detection expires the lock and
  another worker reprocesses it. Because processors upsert idempotently, the
  re-delivery converges rather than orphaning a `running` row.
- **A dump fails** — the row is marked `failed` with `pg_dump`'s stderr, and the
  job throws so BullMQ records it.
- **Redis is flushed** — schedules are re-registered from Postgres on the next
  worker boot.
