...
<Accordion title="Sign-in fails with 'Invalid origin'">
...
Set it on the dashboard to exactly the origin you type in the browser, scheme
included, and restart:

```bash
BASE_URL=https://pgbr.example.com
```

The dashboard logs the value it booted with, so its startup output tells you
whether it picked the variable up.
...
- **Do both services share `DATABASE_URL`?** They must point at the *same*
  database. A dashboard enqueuing into one and a worker polling another looks
  exactly like this — no errors, no progress.
- **Check `job_queue`.** Rows stuck at `pending` mean nothing is claiming;
  rows at `active` with a stale `heartbeat_at` mean a worker died.

The worker logs a ready line on boot. If you don't see it, it isn't reaching
Postgres.
...
- **Is `next_run_at` set?** A null means the schedule hasn't been picked up yet;
  it is filled in on the next tick, without firing.
...
Working as designed. A job whose worker died is requeued once its heartbeat goes
stale, and processors are idempotent so a re-delivery converges on the same row.
