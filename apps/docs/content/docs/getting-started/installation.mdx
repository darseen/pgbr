...
What that file does beyond starting the five containers:

- Postgres, Redis, and SeaweedFS sit on an **internal** network — no published
...
- Redis persists with AOF, so queued jobs survive a restart.
